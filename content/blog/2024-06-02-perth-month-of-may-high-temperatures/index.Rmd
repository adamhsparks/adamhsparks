...
We can see an error with "Medina".
So we can drop that one using indexing, it's the 9th entry, and bind the list into a single data frame using `data.table::rbindlist()`.

```{r drop_md_bind}
library(data.table)
dpird_station_values <- within(dpird_station_values, rm(Medina))
dpird_station_values <- rbindlist(dpird_station_values[-c(9)])
```
...
Again, we can remove the stations by index.
This time, since the list is much longer, use `grep()` to locate the faulty stations and drop them from the list while binding it into a single data frame.
